...
describe("txResultMessageParserFactory-test", () => {
  ...
  it("test parsing to fungibleTokenModifyTxResult", () => {
    ...
    expect("678c146a").to.equal(itemTokenModifyMessage.contractId);
  ...
  it("test parsing to fungibleTokenIssueTxResult", () => {
    ...
    expect(1).to.equal(itemTokenIssueMessage.mintedFungibleTokens.length);
    expect("61e14383").to.equal(
      itemTokenIssueMessage.mintedFungibleTokens[0].contractId,
    );
    expect("00000001").to.equal(
      itemTokenIssueMessage.mintedFungibleTokens[0].tokenType,
    );
    expect("3000").to.equal(
      itemTokenIssueMessage.mintedFungibleTokens[0].amount,
    );
  });
});
