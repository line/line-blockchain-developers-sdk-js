...
describe("txResultMessageParserFactory-test", () => {
  ...
  it("test parsing to serviceTokenIssueTxResult", () => {
    const parser = TxResultMessageParserFactory.create(
      MessageType.SERVICE_TOKEN_ISSUE,
    );
    const serviceTokenIssueMessage = parser.parse(
      issueServiceTokenTxResult,
    ) as ServiceTokenIssueMessage;

    expect("tlink1n9pqyk4jy8d3pd20quryudxw3g47cl99403558").to.equal(
      serviceTokenIssueMessage.from,
    );
    expect("tlink1n9pqyk4jy8d3pd20quryudxw3g47cl99403558").to.equal(
      serviceTokenIssueMessage.to,
    );
    expect("9be17165").to.equal(
      serviceTokenIssueMessage.issuedServiceToken.contractId,
    );
  ...
  it("test parsing to serviceTokenBurnTxResult", () => {
    const parser = TxResultMessageParserFactory.create(
      MessageType.SERVICE_TOKEN_BURN,
    );
  ...
  it("test parsing to serviceTokenBurnFromTxResult", () => {
    const parser = TxResultMessageParserFactory.create(
      MessageType.SERVICE_TOKEN_BURN_FROM,
    );
    const serviceTokenBurnFromMessage = parser.parse(
      serviceTokenBurnFromTxResult,
  ...
  it("test parsing to ServiceTokenApprovedMessage", () => {
    const parser = TxResultMessageParserFactory.create(
      MessageType.SERVICE_TOKEN_PROXY_APPROVED,
    );
    const serviceTokenApprovedMessage = parser.parse(
      serviceTokenProxyApprovedTxResult,
    ) as ServiceTokenApprovedMessage;

    expect("link1j8jd9nps56txm2w3afcjsktrrjh0ft82eftchd").to.equal(
      serviceTokenApprovedMessage.from,
    );
    expect("link1j8jd9nps56txm2w3afcjsktrrjh0ft82eftchd").to.equal(
      serviceTokenApprovedMessage.sender,
    );
    expect("link1he0tp59u36mdjaw560gh8c27pz8fqms88l8nhu").to.equal(
      serviceTokenApprovedMessage.proxy,
    );
    expect("link1j8jd9nps56txm2w3afcjsktrrjh0ft82eftchd").to.equal(
      serviceTokenApprovedMessage.approver,
    );
  ...
  it("test parsing to itemTokenCreateTxResult", () => {
    const parser = TxResultMessageParserFactory.create(
      MessageType.ITEM_TOKEN_CREATE,
    );
    const itemTokenCreateMessage = parser.parse(
      itemTokenCreateTxResult,
    ) as ItemTokenCreateMessage;

    expect("link17k4j8nfr47urlzfz6h7hzdaankpkz0dgce0xkz").to.equal(
      itemTokenCreateMessage.from,
    );
    expect("link17k4j8nfr47urlzfz6h7hzdaankpkz0dgce0xkz").to.equal(
      itemTokenCreateMessage.sender,
    );
    expect("link17k4j8nfr47urlzfz6h7hzdaankpkz0dgce0xkz").to.equal(
      itemTokenCreateMessage.createdItemToken.owner,
    );
    expect("fee15a74").to.equal(
      itemTokenCreateMessage.createdItemToken.contractId,
    );
    ...
    expect("http://test-image-server.com").to.equal(
      itemTokenCreateMessage.createdItemToken.baseImgUri,
    );
  });
  ...
  it("test parsing to itemTokenApproveTxResult", () => {
    const parser = TxResultMessageParserFactory.create(
      MessageType.ITEM_TOKEN_APPROVE,
    );
    const itemTokenApproveMessage = parser.parse(
      itemTokenApproveTxResult,
    ) as ItemTokenApproveMessage;

    expect("link1ygceu3trpkkz9gcyr7m3zzv8n82zd3fawea59p").to.equal(
      itemTokenApproveMessage.approver,
    );
    expect("link17k4j8nfr47urlzfz6h7hzdaankpkz0dgce0xkz").to.equal(
      itemTokenApproveMessage.proxy,
    );
  ...
  it("test parsing to itemTokenDisapproveTxResult", () => {
    const parser = TxResultMessageParserFactory.create(
      MessageType.ITEM_TOKEN_DISAPPROVE,
    );
    const itemTokenDisapproveMessage = parser.parse(
      itemTokenDisapproveTxResult,
    ) as ItemTokenDisapproveMessage;

    expect("link1j8jd9nps56txm2w3afcjsktrrjh0ft82eftchd").to.equal(
      itemTokenDisapproveMessage.approver,
    );
    expect("link1he0tp59u36mdjaw560gh8c27pz8fqms88l8nhu").to.equal(
      itemTokenDisapproveMessage.proxy,
    );
  ...
  it("test parsing to fungibleTokenBurnTxResult", () => {
    ...
    const itemTokenBurnMessage = parser.parse(
      burnFungibleTxResult,
    ) as FungibleTokenBurnMessage;
  ...
  it("test parsing to fungibleTokenBurnFromTxResult", () => {
    const parser = TxResultMessageParserFactory.create(
      MessageType.ITEM_TOKEN_BURN_FROM_FT,
    );
    const itemTokenBurnMessage = parser.parse(
      burnFromFungibleTxResult,
  ...
  it("test parsing to fungibleTokenTransferTxResult", () => {
    const parser = TxResultMessageParserFactory.create(
      MessageType.ITEM_TOKEN_TRANSFER_FT,
    );
    const fungibleTokenTransferMessage = parser.parse(
      fungibleTokenTransferTxResult,
    ) as FungibleTokenTransferMessage;

    expect("tlink1fr9mpexk5yq3hu6jc0npajfsa0x7tl427fuveq").to.equal(
      fungibleTokenTransferMessage.from,
    );
    expect("tlink1fr9mpexk5yq3hu6jc0npajfsa0x7tl427fuveq").to.equal(
      fungibleTokenTransferMessage.sender,
    );
    expect("tlink1nf5uhdmtsshmkqvlmq45kn4q9atnkx4l3u4rww").to.equal(
      fungibleTokenTransferMessage.to,
    );
    expect("61e14383").to.equal(fungibleTokenTransferMessage.contractId);

    expect("61e14383").to.equal(
      fungibleTokenTransferMessage.transferredFungibleTokenAmount.contractId,
    );
    expect("00000001").to.equal(
      fungibleTokenTransferMessage.transferredFungibleTokenAmount.tokenType,
    );
    expect("1").to.equal(
      fungibleTokenTransferMessage.transferredFungibleTokenAmount.amount,
    );
  });

  it("test parsing to fungibleTokenTransferFromTxResult", () => {
    const parser = TxResultMessageParserFactory.create(
      MessageType.ITEM_TOKEN_TRANSFER_FROM_FT,
    );
    ...
    expect("link1j8jd9nps56txm2w3afcjsktrrjh0ft82eftchd").to.equal(
      fungibleTokenTransferFromMessage.from,
    );
    expect("link1he0tp59u36mdjaw560gh8c27pz8fqms88l8nhu").to.equal(
      fungibleTokenTransferFromMessage.proxy,
    );
    expect("link137pmnn2snxdcwa5kmg5rra6u3tf2y5c7emmm7p").to.equal(
      fungibleTokenTransferFromMessage.to,
    );
    expect("bf365bab").to.equal(fungibleTokenTransferFromMessage.contractId);

    expect("bf365bab").to.equal(
      fungibleTokenTransferFromMessage.transferredFungibleTokenAmount
        .contractId,
    );
    expect("00000001").to.equal(
      fungibleTokenTransferFromMessage.transferredFungibleTokenAmount.tokenType,
    );
    expect("50").to.equal(
      fungibleTokenTransferFromMessage.transferredFungibleTokenAmount.amount,
    );
  });
  ...
  it("test parsing to nonFungibleTypeIssueTx", () => {
    ...
    const itemTokenBurnMessage = parser.parse(
      issueNonFungibleTypeTxResult,
    ) as NonFungibleTokenIssueMessage;
    ...
    expect("61e14383").to.equal(itemTokenBurnMessage.contractId);
    expect("61e14383").to.equal(
      itemTokenBurnMessage.issuedNonFungibleToken.contractId,
    );
    expect("1000000c").to.equal(
      itemTokenBurnMessage.issuedNonFungibleToken.tokenType,
    );
  });
  ...
  it("test parsing to nonFungibleTypeMintTx", () => {
    ...
    const itemTokenBurnMessage = parser.parse(
      mintNonFungibleTxResult,
    ) as NonFungibleTokenMintMessage;
    ...
    expect("61e14383").to.equal(
      itemTokenBurnMessage.mintedNonFungibleTokens[0].contractId,
    );
    expect("10000001").to.equal(
      itemTokenBurnMessage.mintedNonFungibleTokens[0].tokenType,
    );
    expect("00000007").to.equal(
      itemTokenBurnMessage.mintedNonFungibleTokens[0].tokenIndex,
    );
  });
  ...
  it("test parsing to nonFungibleTypeMultiMintTx", () => {
    ...
    expect("61e14383").to.equal(
      itemTokenBurnMessage.mintedNonFungibleTokens[1].contractId,
    );
    expect("10000001").to.equal(
      itemTokenBurnMessage.mintedNonFungibleTokens[0].tokenType,
    );
    expect("10000002").to.equal(
      itemTokenBurnMessage.mintedNonFungibleTokens[1].tokenType,
    );
    expect("0000000a").to.equal(
      itemTokenBurnMessage.mintedNonFungibleTokens[0].tokenIndex,
    );
    expect("00000001").to.equal(
      itemTokenBurnMessage.mintedNonFungibleTokens[1].tokenIndex,
    );
  });
  ...
  it("test parsing to nonFungibleTypeBurnTx", () => {
    ...
    const itemTokenBurnMessage = parser.parse(
      burnNonFungibleTxResult,
    ) as NonFungibleTokenBurnMessage;
    ...
    expect("61e14383").to.equal(itemTokenBurnMessage.contractId);
    expect("61e14383").to.equal(
      itemTokenBurnMessage.burnedNonFungibleToken.contractId,
    );
    expect("10000001").to.equal(
      itemTokenBurnMessage.burnedNonFungibleToken.tokenType,
    );
    expect("00000003").to.equal(
      itemTokenBurnMessage.burnedNonFungibleToken.tokenIndex,
    );
  });

  it("test parsing to nonFungibleTypeBurnFromTx", () => {
    const parser = TxResultMessageParserFactory.create(
      MessageType.ITEM_TOKEN_BURN_FROM_NFT,
    );
    const itemTokenBurnFromMessage = parser.parse(
      burnFromNonFungibleTxResult,
    ) as NonFungibleTokenBurnFromMessage;

    expect("tlink17dz3hqn6nd5j6euymaw3ft9phgspmuhfjqazph").to.equal(
      itemTokenBurnFromMessage.from,
    );
    expect("tlink1fr9mpexk5yq3hu6jc0npajfsa0x7tl427fuveq").to.equal(
      itemTokenBurnFromMessage.proxy,
    );
    expect("61e14383").to.equal(itemTokenBurnFromMessage.contractId);
    expect("61e14383").to.equal(
      itemTokenBurnFromMessage.burnedNonFungibleToken.contractId,
    );
    expect("10000001").to.equal(
      itemTokenBurnFromMessage.burnedNonFungibleToken.tokenType,
    );
    expect("00000005").to.equal(
      itemTokenBurnFromMessage.burnedNonFungibleToken.tokenIndex,
    );
  });

  it("test parsing to nonFungibleTypeTransferTx", () => {
    const parser = TxResultMessageParserFactory.create(
      MessageType.ITEM_TOKEN_TRANSFER_NFT,
    );
    const itemTokenTransferMessage = parser.parse(
      transferNonFungibleTxResult,
    ...
    expect("00000006").to.equal(
      itemTokenTransferMessage.transferredNonFungibleTokens[1].tokenIndex,
    );
  });

  it("test parsing to nonFungibleTypeTransferFromTx", () => {
    const parser = TxResultMessageParserFactory.create(
      MessageType.ITEM_TOKEN_TRANSFER_FROM_NFT,
    );
    ...
    expect("link1j8jd9nps56txm2w3afcjsktrrjh0ft82eftchd").to.equal(
      itemTokenTransferFromMessage.from,
    );
    expect("link1he0tp59u36mdjaw560gh8c27pz8fqms88l8nhu").to.equal(
      itemTokenTransferFromMessage.proxy,
    );

    expect("bf365bab").to.equal(
      itemTokenTransferFromMessage.transferredNonFungibleTokens[0].contractId,
    );
    expect("bf365bab").to.equal(
      itemTokenTransferFromMessage.transferredNonFungibleTokens[1].contractId,
    );
    expect("10000001").to.equal(
      itemTokenTransferFromMessage.transferredNonFungibleTokens[0].tokenType,
    );
    expect("10000001").to.equal(
      itemTokenTransferFromMessage.transferredNonFungibleTokens[1].tokenType,
    );
    expect("0000000e").to.equal(
      itemTokenTransferFromMessage.transferredNonFungibleTokens[0].tokenIndex,
    );
    expect("0000000f").to.equal(
      itemTokenTransferFromMessage.transferredNonFungibleTokens[1].tokenIndex,
    );
  });

  it("test parsing to baseCoinSendTx", () => {
    const parser = TxResultMessageParserFactory.create(MessageType.COIN_SEND);
    const baseCoinTransferMessage = parser.parse(
      baseCoinTransferTxResult,
    ) as BaseCoinTransferMessage;

    expect("tlink1fr9mpexk5yq3hu6jc0npajfsa0x7tl427fuveq").to.equal(
      baseCoinTransferMessage.from,
    );
    expect("tlink1fr9mpexk5yq3hu6jc0npajfsa0x7tl427fuveq").to.equal(
      baseCoinTransferMessage.sender,
    );
